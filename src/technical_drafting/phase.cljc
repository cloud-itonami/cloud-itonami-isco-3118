(ns technical-drafting.phase
  "The verdict -> phase mapping for the ISCO-08 3118 technical draughtsperson
  ...
  drawing revision is written, held, or sent to a human had no test of its own
  ...
  most likely to be waved through: a revision of a drawing that is already
  issued for construction escalates by declaration, and can at the same time
  cite a superseded drawing, name another project's drawing, or carry a
  reserved op such as `:seal-and-stamp`. `:seal-and-stamp` is the case that
  matters — a human asked to approve it would be approving the actor applying
  a professional seal, which is not theirs to delegate.

  Note precisely what this ordering does and does not currently protect.
  `technical-drafting.governor` computes `:escalate?` as `(and (not hard?)
  ...
  holds for any caller building a verdict by hand or for a future governor that
  stops zeroing the flag. It is therefore covered by a unit test over this pure
  function and NOT by `technical-drafting.sim` — reversing the two clauses
  ...
  (:require [technical-drafting.operation :as op]))
...
(defn refusal?
  ...
  without a human, not an approval-in-waiting. `technical-drafting.sim` counts
...
(defn approved-commit?
  ...
  pre-change tree, it could not: a specification discrepancy flagged after
  human sign-off and a routine drawing draft committed automatically left two
  `{:disposition :commit ...}` entries with no field telling them apart. For a
  record whose whole purpose is to show who authorised a change to a drawing
  people build from, that distinction is the entire reason the interrupt
  exists."
  [disposition]
...
(defn escalating-op?
  ...
  than by confidence. Re-exported from `technical-drafting.operation` so a
  reader of the routing rule can see which ops always take the human path
  without following a second namespace."
  [o]
  (op/escalates? o))
