(ns technical-drafting.ledger
  "Append-only audit ledger for the ISCO-08 3118 technical draughtsperson
  ...
  exactly what an audit is trying to check. For the record of which drawing
  revisions were approved on a project that people build from, that is the
  difference between an audit that can show what was approved and one that can
  only show what someone chose to leave in the file.
  ...
  specification discrepancy flagged through human sign-off and a routine
  drawing draft committed automatically both produced
  `{:disposition :commit :record ...}` with no field distinguishing them.
  `commit-entry` therefore takes the approval provenance as part of the entry,
...
(defn summary
  "One line per entry, for operators and for `technical-drafting.sim` output."
  [ledger]
  (str/join "\n"
            (for [e ledger]
